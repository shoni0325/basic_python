...
# 결측치를 확인하는 방법
is.na(df)
table(is.na(df))      # table은 갯수 세는 것 (python의 value_counts와 유사)
is.na(df$c1)

# dplyr 패키지를 이용하여 결측치 데이터를 확인하는 방법
df %>% filter(!is.na(c1))

na.omit(df)           # NA가 존재하는 인덱스 삭제(결측치를 완벽하게 제거하지만 다소 위험하다)

# 결측치를 제외하고 연산
mean(df$c1)
mean(df$c2)
mean(df$c3)
mean(df$c1, na.rm = T)  # rm : remove의 약자

exam = read.csv("./shoni_python/csv/csv_exam.csv")
