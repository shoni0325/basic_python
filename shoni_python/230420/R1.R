# csv 파일을 로드
df = read.csv("./shoni_python/csv/example.csv")
df
...
# 컬럼의 이름 변경 (after = before)
rename(df, '이름' = Name)

df = read.csv("./shoni_python/csv/csv_exam.csv")
df
...
df = cbind(df, total_score, mean_score)
View(df)
...
rbind(df1, df2)       # 컬럼명이 달라서, 합쳐지지 않는다
bind_rows(df1, df2)   # 그래도 강제로 합치고 싶을 때 유니언 결합 같은 의미